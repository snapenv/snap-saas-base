...
RUN python -m venv /opt/snap-saas-base-env
ENV PATH /opt/snap-saas-base-env/bin:$PATH
ENV VIRTUAL_ENV /opt/snap-saas-base-env

# Set the working directory.
WORKDIR /workspaces/snap-saas-base/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/snap-saas-base/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/snap_saas_base/ && touch src/snap_saas_base/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/snap-saas-base/
